...
<Callout title="Note">
  This may seem a bit strange. How will Cloudflare no what to do with
  `100.118.49.101`? This is an internal IP address that is only accessible from
  within your homelab. The trick is that Cloudflare does not care! We just need
  the DNS name `edwardes.xyz` to resolve to this IP address. When I am on my
  home network it will resolve to my server hosting Caddy. When I am not inside
  my home network it will still resolve to `100.118.49.101`, but this IP address
  will mean nothing to the outside world.
</Callout>
