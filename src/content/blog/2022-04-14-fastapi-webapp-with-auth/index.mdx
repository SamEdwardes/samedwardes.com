...
A _"private"_ page that can only be viewed by users who are logged in:
...
Next, set up your _requirements.txt_ file:
...
<Tabs>
  ...
  <Fragment slot="tab-content">
    <TabContent>
      <LocalCodeChunk
        src="/blog-assets/2022-04-14-fastapi-webapp-with-auth/requirements.txt"
        lang="html"
      />
    </TabContent>
    <TabContent>
      <LocalCodeChunk
        src="/blog-assets/2022-04-14-fastapi-webapp-with-auth/main.py"
        lang="python"
      />
    </TabContent>
    <TabContent>
      <LocalCodeChunk
        src="/blog-assets/2022-04-14-fastapi-webapp-with-auth/templates/index.html"
        lang="html"
      />
    </TabContent>
    <TabContent>
      <LocalCodeChunk
        src="/blog-assets/2022-04-14-fastapi-webapp-with-auth/templates/login.html"
        lang="html"
      />
    </TabContent>
    <TabContent>
      <LocalCodeChunk
        src="/blog-assets/2022-04-14-fastapi-webapp-with-auth/templates/private.html"
        lang="html"
      />
    </TabContent>
    <TabContent>
      <LocalCodeChunk
        src="/blog-assets/2022-04-14-fastapi-webapp-with-auth/templates/_site_map.html"
        lang="html"
      />
    </TabContent>
  </Fragment>
...
We have also created a function called `get_user`. This function will be used in many places throughout to access users from our _"database"_.
...
- We the line `token = token.removeprefix("Bearer").strip()` removes the string _"Bearer"_ from the start of token string. This will be necessary when we are call `decode_token()` from within the `get_current_user_from_cookie()` function.
...
<Callout title="Tip">
  You can see the cookies that are currently set in your browser using your
  browser's built in developer tools. ![screenshot](./imgs/cookie.png)
</Callout>
...
- We create a class `LoginForm` to handle the logic of validating the form data. In the event that the submitted form data is not valid we will re-render the _"templates/login.html"_ with the same form data and an error message.
