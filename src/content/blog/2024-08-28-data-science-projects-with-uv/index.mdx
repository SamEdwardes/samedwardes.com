...
To upgrade a specific package, use the `uv lock --upgrade-package <pkg-name>` command. `uv lock` will only update your lockfile. Run `uv sync` to update your virtual environment to match the lockfile.
